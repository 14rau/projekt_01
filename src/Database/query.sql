...
LEFT JOIN product_requirements AS pr ON p.id = pr.product_id
...
WHERE manufacturer_id =(SELECT id FROM manufacturer WHERE name = [manufacturer_name]);
...
SELECT product_name, count(designation_id) FROM product 
LEFT JOIN product_designation ON product.designation_id = product_designation.id
GROUP BY product.designation_id
